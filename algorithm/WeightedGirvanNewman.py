...
class WeightedGirvanNewman:
    def __init__(self, G, weightname):
        self.G_copy = G.copy()
        self.G = G
    ...
    def divByK(self, k):
        ...
        while len(self.G.edges()) != 0:
            ...
            for e, ebc in edges_betweenness_centrality.items():
                edge_bc_w = ebc / self.G.get_edge_data(e[0],
                                                       e[1])[self.weight_name]
                edge_bc_ws[e] = edge_bc_w
            # Find the edge with largest betweenness centrality (divided by weight)
            max_bc_w_edge = max(edge_bc_ws.items(), key=lambda item: item[1])[0]
            # Remove the edge with largest betweenness centrality
            self.G.remove_edge(max_bc_w_edge[0], max_bc_w_edge[1])
            print("Removed an edge: (" + str(max_bc_w_edge[0]) + ", " +
                  str(max_bc_w_edge[1]) + ")")
...
if __name__ == '__main__':
    ...
    wgngraph = WeightedGirvanNewman(G, "weight")
    wgngraph.divByK(20)
    wgngraph.drawQ()